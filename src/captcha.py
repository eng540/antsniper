"""
Elite Sniper v3.6 - Production Grade Captcha System
Refactored for 100% Local OCR (ddddocr) & State-Aware Synchronization
[FIX] Context-Aware Refresh: Handles page reload gracefully without crashing.
[FIX] OpenCV: Non-destructive filtering for better accuracy.
"""
...
class EnhancedCaptchaSolver:
    ...
    - [FIX] Robust Context Handling during Refresh (V3.6)
    """
    ...
    def _extract_base64_captcha(self, page: Page, location: str = "EXTRACT") -> Optional[bytes]:
        try:
            ...
            for attempt in range(max_attempts):
                # Critical: Protect against destroyed context during polling
                try:
                    ...
                    if "destroyed" in str(e) or "navigating" in str(e):
                         logger.debug(f"[{location}] Context refreshed during polling - retrying...")
                         time.sleep(0.5)
                         # Re-acquire element
                         try: captcha_div = page.locator("captcha > div").first
                         except: pass
                         continue
                    raise e
    ...
    def get_valid_captcha_turbo(self, page: Page, location: str = "BOOKING_TURBO") -> Optional[str]:
        """
        [FIXED] Context-Aware Sniper Protocol
        يعالج خطأ 'Execution context destroyed' عند تحديث الكابتشا ولا ينهار.
        """
        max_retries = 30
        REFRESH_ID = "appointment_newAppointmentForm_form_newappointment_refreshcaptcha"
        
        # انتظار أولي لظهور العنصر
        try:
            page.wait_for_selector("captcha > div", state="attached", timeout=12000)
        except Exception as e:
            logger.error(f"[{location}] Captcha element failed to appear: {e}")
            return None

        for attempt in range(max_retries):
            try:
                # [CRITICAL FIX] حماية الاستعلام من انهيار السياق
                try:
                    element = page.query_selector("captcha > div")
                except Exception as ctx_error:
                    if "destroyed" in str(ctx_error) or "navigating" in str(ctx_error):
                        logger.warning(f"[{location}] Context reloading... waiting.")
                        time.sleep(0.5)
                        continue
                    raise ctx_error
                ...
                if len(result) != 6:
                    ...
                    # محاولة ضغط زر التحديث بأمان مع توقع الانهيار
                    try:
                        page.evaluate(f"""
                            var btn = document.getElementById('{REFRESH_ID}');
                            if(btn) btn.click();
                        """)
                        # ننتظر وقتاً كافياً ليتم تدمير السياق وبناؤه من جديد
                        time.sleep(1.5) 
                    except Exception as refresh_error:
                        # هذا الخطأ متوقع لأن الصفحة تغادر
                        logger.debug(f"Refresh triggered context loss (Expected): {refresh_error}")
                        time.sleep(1.0)
                ...
                # إذا حدث خطأ تدمير السياق هنا أيضاً، ننتظر ونكمل
